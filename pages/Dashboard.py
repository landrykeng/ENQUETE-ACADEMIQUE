...
# Nouvelle base de données
        data_enquete = pd.read_excel("Simulation.xlsx", sheet_name="Données Collectées")
        data_enquete_rep = pd.read_excel("Simulation.xlsx", sheet_name="Distribution Ménages")
        ...
        with cl_tb[1]:
            ...
            unsafe_allow_html=True)
        with cl_tb[2]:
            st.image(logo2,caption="",width=200)
        ...
        with tabs[0]:
            ...
            with ca[1]:
                display_single_metric_advanced(" 🗑️ dechet menagers", 159, delta=13, color_scheme="green")
            with ca[2]:
            ...
            with col[1]:
                ...
                with subcol[1]:
                    st.write("")
                    make_dbl_progress_char(["💰","🗑️ "],[progress_enf,progress_ens],colors=["#4b8bff","#ff4b4b"],titre=traduire_texte("Progression par type de questionnaire ",lang),height=323)
            ...
            with col[0] :
                type_questionnaire=st.selectbox("Type de Questionnaire", options=["Tontine", "Dechet ménager"])
                ...
                graphique_barre_croise_effectifs(data_enquete,x_col="arrondissement",group_col="id_superviseur")
        ...
        with tabs[1]:
            ...
            with col1[1]:
                ...
                with sbcl[1]:
                    ...
                    controleur=st.multiselect(traduire_texte("Sélectionner le controleur",lang),data_superviz["Controleur"].unique(),default=data_superviz["Controleur"].unique())
                    data_controleur = data_superviz[data_superviz["Controleur"].isin(controleur)]
            ...
            with col1[0]:
                ...
                make_cross_hist_b(data_controleur,var2="Enqueteur",palette=palette[6:],var1="Type",titre=traduire_texte("Charge de travail accomplie par enquêteur",lang),typ_bar=2)
            tcol=st.columns(2)
            with tcol[0]:
            ...
            with cltime[1]:
                Sup_time=st.multiselect("SUPERVISEUR", options=All_data["Superviseur"].unique())
